...
def readData(filename):
    ...
    with open(filename, 'r') as csvFile: #Opeing the file 
        ...
        for row in file: #Looping through the file and adding each line as a string to the list. 
            ...
            listofTemps.append((rowSplit[0]))
            listofWords.append((rowSplit[1]))
    return listofTemps,listofWords
...
if __name__ == "__main__":  
    ...
    messageList1, messageList2 = readData("randData.csv")

    for i in range(len(messageList1)):
    # send the message to the queue
        send_message("localhost","tempQueue",messageList1[i])
        send_message("localhost","wordQueue", messageList2[i])